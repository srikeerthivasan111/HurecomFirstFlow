...
public class Baseclass {
	...
	protected void screenbyte(Scenario scenario) {

		TakesScreenshot tk = (TakesScreenshot) driver;
		byte[] screenshotAs = tk.getScreenshotAs(OutputType.BYTES);
		System.out.println("driver is complete");

		scenario.attach(screenshotAs, "image/png", "keerthi");

	}

	public static WebDriver browserlaunchernew(String browserusername) {

		if (browserusername.equals("chrome")) {

			WebDriverManager.chromedriver().setup();
			driver = new ChromeDriver();
		}

		else if (browserusername.equals("edge")) {
			WebDriverManager.edgedriver().setup();
			driver = new EdgeDriver();
		}

		else if (browserusername.equals("firefox")) {
			WebDriverManager.edgedriver().setup();
